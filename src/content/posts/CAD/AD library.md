---
title: How to Build and Maintain a Library in AD
...
![create1](AD%20library.assets/create1.png)
![create2](AD%20library.assets/create2.png)
![create3](AD%20library.assets/create3.png)
以上三图：建立一个Intlib，并添加一个Schlib。

### 添加原理图符号
![addsch1](AD%20library.assets/addsch1.png)

**在开始画之前，务必确认工作区当前的Grid大小**，可以窗口最左下角的信息栏中看到这个数值。我的习惯是以100mil为最小单位画图，所以我原理图符号的引脚都会在以100mil为间隔的格点上。如果引脚在原理图上出现在150mil或3,999mil这样的位置上，就不可能被任何导线连接。恐怖的是有时候自己注意不到这个问题。

好消息是，需要动手画的元件无非就是RLC和某些变压器（其实变压器没有什么复用性，一般会出现在各个工程自己的小库里）。

对于大部分连接器或IC，可以用AD的Symbol Wizard工具快速生成原理图符号：

![addsch2](AD%20library.assets/addsch2.png)
...
### 将封装绑定到原理图符号
![binding1](AD%20library.assets/binding1.png)
![binding2](AD%20library.assets/binding2.png)

### 编译
![compile](AD%20library.assets/compile.png)

### （可选）将编译好的库添加到AD环境中
![environment1](AD%20library.assets/environment1.png)
![environment2](AD%20library.assets/environment2.png)